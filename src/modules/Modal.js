import { createElement, getAssetPath } from './Utilities';
...
function createModalContactsContent() {
  ...
  emailIcon.setAttribute('src', getAssetPath('src/img/icons/mail.svg'));
  ...
  telegramIcon.setAttribute('src', getAssetPath('src/img/icons/telegram.svg'));
  ...
  phoneIcon.setAttribute('src', getAssetPath('src/img/icons/phone.svg'));
  phoneIcon.setAttribute('alt', 'Phone icon');

  const phoneText = document.createTextNode(
    'Телефон:\u00A0\u00A0 +79158326698\u00A0\u00A0(Сергей)',
  );
